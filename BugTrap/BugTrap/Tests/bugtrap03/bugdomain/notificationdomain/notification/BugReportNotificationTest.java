...
public class BugReportNotificationTest {
	...
	BugReportNotification testNot3 = new BugReportNotification(message4Not,
	        new BugReport(dev4Not, "a", "b", new GregorianCalendar(), PList.<BugReport> empty(), subsystem4MB,
	                new Milestone(1, 2, 4), 1, false, "d", "e", "f"),
	        project4MB);
	assertFalse(testNot.equals(testNot3));
	BugReportNotification testNot4 = new BugReportNotification(message4Not, bugreport4MB,
	        new Project("g", "h", dev4Not, 123));
	assertFalse(testNot.equals(testNot4));

	Notification notif = new Notification("Test", project4MB);
	assertFalse(testNot.equals(notif));
    }
